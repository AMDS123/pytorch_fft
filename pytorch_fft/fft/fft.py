...
def fft(X_re, X_im): 
    if not(X_re.dim() >= 2 and X_im.dim() >= 2): 
...
def ifft(X_re, X_im): 
    ...
    if 'Float' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).cuda() for _ in range(2))
        th_fft.th_Float_ifft2(X_re, X_im, Y1, Y2, 1)
        return (Y1/N, Y2/N)
    elif 'Double' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).double().cuda() for _ in range(2))
        th_fft.th_Double_ifft2(X_re, X_im, Y1, Y2, 1)
        return (Y1/N, Y2/N)
    else:
        raise NotImplementedError
...
def fft2(X_re, X_im): 
    ...
    if 'Float' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).cuda() for _ in range(2))
        th_fft.th_Float_fft2(X_re, X_im, Y1, Y2, 2)
        return (Y1, Y2)
    elif 'Double' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).double().cuda() for _ in range(2))
        th_fft.th_Double_fft2(X_re, X_im, Y1, Y2, 2)
        return (Y1, Y2)
    else:
        raise NotImplementedError
...
def ifft2(X_re, X_im): 
    ...
    if 'Float' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).cuda() for _ in range(2))
        th_fft.th_Float_ifft2(X_re, X_im, Y1, Y2, 2)
        return (Y1/N, Y2/N)
    elif 'Double' in type(X_re).__name__: 
        Y1, Y2 = tuple(torch.zeros(*X_re.size()).double().cuda() for _ in range(2))
        th_fft.th_Double_ifft2(X_re, X_im, Y1, Y2, 2)
        return (Y1/N, Y2/N)
    else:
        raise NotImplementedError
